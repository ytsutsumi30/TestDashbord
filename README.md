...
このコマンドは `TestDashboard\.env` と `..\.env.azure` の `TESTDASHBOARD_API_KEY` を同じ値に揃えます。
既存キーがあれば保持し、未設定なら新規生成します。ローテーションする場合は `node scripts/setup-api-key.js --rotate` を実行してください。
Androidアプリは設定画面の「サーバーAPIキー」に同じ値を入力します。
GitHub Pages版はBackend URL設定ダイアログのAPIキー欄、または初回だけ `?api_key=<key>` 付きURLで開くとブラウザに保存されます。

### Queue 再試行とDLQ

Teams webhook由来のQueue処理は、`QUEUE_MAX_DEQUEUE_COUNT` 回失敗すると元Queueから削除し、`QUEUE_DLQ_NAME` へ退避します。
